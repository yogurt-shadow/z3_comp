...
// wzh
    lbool kissing_check(){
        clause_vector arith_clauses;
        for(clause * cls: m_clauses){
            var x = max_var(*cls);
            if(x != null_var){
                arith_clauses.push_back(cls);
            }
        }
        Kissing_Checker checker(m_pm, arith_clauses, m_atoms);
        return checker.get_answer();
    }
// hzw
