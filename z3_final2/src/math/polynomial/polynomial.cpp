...
#define TMP_INITIAL_CAPACITY 128
    ...
    */
    ...
    int manager::sign_lower_inf(polynomial const * p, var x){
        ...
        if(const_coeff(p, x, deg, curr)){
            bool positive1 = (deg % 2 == 0);
            bool positive2 = (m().is_pos(curr));
            return positive1 == positive2 ? 1 : -1;
        }
        else {
            UNREACHABLE();
        }
    ...
    int manager::sign_upper_inf(polynomial const * p, var x){
        unsigned deg = degree(p, x);
        scoped_numeral curr(m());
        if(const_coeff(p, x, deg, curr)){
            return m().is_pos(curr) ? 1 : -1;
        }
        else {
            UNREACHABLE();
        }
    ...
    unsigned manager::sturm(polynomial const * p, var x) {
        if(is_const(p)){
            return 0;
        }
        SASSERT(is_univariate(p) && x == max_var(p));
        ptr_vector<polynomial> vec;
        sturm_sequence(p, x, vec);
        if(vec.size() <= 1){
            return 0;
        }
        int last1 = sign_lower_inf(vec[0], x), last2 = sign_upper_inf(vec[0], x);
        unsigned change1 = 0, change2 = 0;
        for(unsigned i = 1; i < vec.size(); i++){
            int curr1 = sign_lower_inf(vec[i], x), curr2 = sign_upper_inf(vec[1], x);
            if(curr1 != last1){
                change1++;
            }
            if(curr2 != last2){
    ...
    void manager::sturm_sequence(polynomial const * p, var x, ptr_vector<polynomial> & vec){
        vec.reset();
        vec.push_back(const_cast<polynomial *>(p));
        polynomial * p_d = derivative(p, x);
        vec.push_back(const_cast<polynomial *>(p_d));
        if(is_const(p_d)){
            return;
        }
        polynomial_ref next(*this);
        do {
            polynomial * p1 = vec[vec.size() - 2];
            polynomial * p2 = vec[vec.size() - 1];
            exact_pseudo_remainder(p1, p2, next);
