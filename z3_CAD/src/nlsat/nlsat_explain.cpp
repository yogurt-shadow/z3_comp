...
namespace nlsat {
    ...
    struct explain::imp {
        ...
        // wzh
        bool m_project_eq;
        polynomial_ref_vector m_eqs;
        polynomial_ref_vector m_chosen_eqs;
        var_vector m_vars;
        var_vector m_index;
        var m_max_var;
        ...
        imp(solver & s, assignment const & x2v, polynomial::cache & u, atom_vector const & atoms, atom_vector const & x2eq,
            ...
            // wzh
            m_eqs(m_pm),
            m_equal(s, m_pm, m_cache),
            m_chosen_eqs(m_pm),
            m_cm(s, m_am, m_pm, ev, ev.ism(), m_cache, MCCALLUM),
            m_ppm(m_pm, m_am, m_assignment, s, false)
            // hzw
            {
        ...
        */
        ...
        void add_cell_lits_sample(polynomial_ref_vector & ps, var y, polynomial_ref_vector & samples){
            samples.reset();
            SASSERT(m_assignment.is_assigned(y));
            bool lower_inf = true;
            bool upper_inf = true;
            ...
            if (!lower_inf) {
                samples.push_back(p_lower);
                add_root_literal(m_full_dimensional ? atom::ROOT_GE : atom::ROOT_GT, y, i_lower, p_lower);
            }
            if (!upper_inf) {
                samples.push_back(p_upper);
                add_root_literal(m_full_dimensional ? atom::ROOT_LE : atom::ROOT_LT, y, i_upper, p_upper);
            }
        }
        // hzw sample projection

        // wzh nlcac
        // in cac case, we return an interval which is acquired 
        // by evaluating current assignment to the polynomials
        void add_cac_cell_lits(polynomial_ref_vector & ps, var y) {
            SASSERT(m_assignment.is_assigned(y));
            bool lower_inf = true;
            bool upper_inf = true;
            scoped_anum_vector & roots = m_roots_tmp;
            ...
            for (unsigned k = 0; k < sz; k++) {
                ...
                for (unsigned i = 0; i < num_roots; i++) {
                    ...
                    if (s == 0) {
                        ...
                        // ! (y = root_i(p))
                        // add_root_literal(atom::ROOT_EQ, y, i+1, p);
                        m_cm.add_cac_ineq_atom(atom::EQ, y, y_val, *m_result, m_already_added_literal);
                        return;
                    }
                    else if (s < 0) {
                        // y_val < roots[i]
            ...
            // we just return !(a, b)
            if(!lower_inf){
                m_cm.add_cac_ineq_atom(atom::GT, y, lower, *m_result, m_already_added_literal);
            }
            if(!upper_inf){
                m_cm.add_cac_ineq_atom(atom::LT, y, upper, *m_result, m_already_added_literal);
            }
        }
        // hzw nlcac
        ...
        */
        ...
        // wzh sample projection
        void project_sample(polynomial_ref_vector & ps, var max_x){
            if(ps.empty()){
                return;
            }
            m_todo.reset();
            ...
            if (x < max_x)
                add_cell_lits_sample(ps, x, samples);
            while (true) {
        ...
        // wzh nlcac
        void project_nlcac(cinterval_vector & ps, var max_x){
            if(ps.empty()){
                return;
            }
            var x = max_x;
            if(m_cm.all_univ(ps, x)){
                return;
            }
            TRACE("wzh", tout << "[nlcac] show cinterval vector in project" << std::endl;
                m_cm.display(tout, ps);
            );
            polynomial_ref_vector polys(m_pm);
            m_cm.construct_characterization(ps, polys, max_x, m_assignment);
            TRACE("wzh", tout << "[nlcac] show polynomials after construction:\n";
                for(auto ele: polys){
                    m_pm.display(tout, ele);
                }
                tout << std::endl;
            );
            var next_v = max_var(polys);
            // TODO: how do we update cinterval for more projection levels?

            TRACE("wzh", tout << "[nlcac] show result lemma before cac: " << std::endl;
                if(m_result == nullptr){
            ...
            // in cac version, we add evaluated intervals
            // add_cac_cell_lits(polys, next_v);
            add_cell_lits(polys, next_v);

            TRACE("wzh", tout << "[nlcac] show result lemma after cac: " << std::endl;
                if(m_result == nullptr){
                    tout << "empty scoped literal vector" << std::endl;
                }
                else{
            ...
            // currently we use partial projection with level = var - 2

            if(m_vars.size() > 2){
                m_ppm.mk_partial_lemma(m_vars, m_index, m_vars.size() - 2, *m_result, m_already_added_literal);
            }

            TRACE("wzh", tout << "[nlcac] show result lemma totally: " << std::endl;
                if(m_result == nullptr){
                    tout << "empty scoped literal vector" << std::endl;
                }
                else{
        ...
        // wzh Equational Constraints
        // eqs is a subset of ps
        void project_EQ(polynomial_ref_vector & ps, polynomial_ref_vector & eqs, var max_x){
            if (ps.empty())
                return;
            m_todo.reset();
            for (poly* p : ps) {
            ...
            TRACE("wzh", tout << "[equal project] Equational Constraints: \n";
                display(tout, eqs);
                tout << std::endl;
            );
            ...
            while (true) {
                ...
                TRACE("nlsat_explain", tout << "[equal project] project loop, processing var "; display_var(tout, x); tout << "\npolynomials\n";
                      display(tout, ps); tout << "\n";);
                
                polynomial_ref curr_eq(m_pm);
                curr_eq = eqs.get(m_index[x]);

                // Projection
                if(curr_eq == nullptr){
                    TRACE("wzh", tout << "[equal project] do not have equational constraints on var "; display_var(tout, x);
                        tout << std::endl;
                    );
                    add_lc(ps, x);
                    psc_discriminant(ps, x);
                    ...
                    TRACE("wzh",  tout << "use EQ\n";
                    tout << "[equal project] equational constraints on var "; display_var(tout, x);
                        tout << std::endl;
                        display(tout, curr_eq);
                        tout << std::endl;
                    );
                    polynomial_ref_vector curr_eqs(m_pm);
                    curr_eqs.push_back(curr_eq);
                    if(m_index[x] == 1 || m_index[x] == m_index.size() - 1){
                        project_sub(curr_eqs, ps, x);
                    }
                    else {
                        project_sub_star(curr_eqs, ps, x);
                    }
                }
                if (m_todo.empty()){
                    break;
                ...
                x = m_todo.remove_max_polys(ps);
                // Lifting
                var next_x = m_index[x];
                polynomial_ref next_eq(m_pm);
                next_eq = eqs.get(next_x);
                if(next_eq == nullptr){
                    add_cell_lits(ps, x);
                }
                else{
                    polynomial_ref_vector next_eqs(m_pm);
                    next_eqs.push_back(next_eq);
                    add_cell_lits(next_eqs, x);
                }
            }
        }

        // assume ps belongs to ps2
        void project_sub(polynomial_ref_vector & ps, polynomial_ref_vector & ps2, var max_x){
            TRACE("wzh", tout << "[equal project] Enter sub projection\n";);
            TRACE("wzh", 
                tout << "[equal project] Equational Constraints: \n";
                display(tout, ps);
                tout << std::endl;
                tout << "[equal project] All constraints: \n";
                display(tout, ps2);
                tout << std::endl;
            );

            add_lc(ps, max_x);
            psc_discriminant(ps, max_x);
            psc_resultant(ps, max_x);
            polynomial_ref_vector comple(m_pm);
            complement(ps, ps2, comple);
            polynomial_ref p(m_pm), q(m_pm);
            for(unsigned i = 0; i < ps.size(); i++){
                p = ps.get(i);
                for(unsigned j = 0; j < comple.size(); j++){
                    q = comple.get(j);
                    psc(p, q, max_x);
                }
            }
        ...
        // ps is a subset of ps2
        void project_sub_star(polynomial_ref_vector & ps, polynomial_ref_vector & ps2, var max_x){
            TRACE("wzh", tout << "[equal project] Enter sub star projection\n";);
            TRACE("wzh", 
                tout << "[equal project] Equational Constraints: \n";
                display(tout, ps);
                tout << std::endl;
            ...
            psc_discriminant(comple, max_x);
        }

        bool is_subset(polynomial_ref_vector const & lhs, polynomial_ref_vector const & rhs){
            for(auto ele: lhs){
                if(!rhs.contains(ele)){
                    return false;
                }
        ...
        // ps1 belongs to ps2
        // get complement of ps1
        void complement(polynomial_ref_vector & ps1, polynomial_ref_vector & ps2, polynomial_ref_vector & res){
            SASSERT(is_subset(ps1, ps2));
            res.reset();
            for(auto ele: ps2){
                if(!ps1.contains(ele)){
                    res.push_back(ele);
        ...
        // wzh partial projection
        void project_partial(polynomial_ref_vector & ps, var max_x) {
            if (ps.empty())
                return;
            m_todo.reset();
            for (poly* p : ps) {
            ...
            var level_x = m_partial_project ? m_ppm.heuristic_partial_level(m_vars, m_index) : 0;
            var x = m_todo.remove_max_polys(ps);
            // Remark: after vanishing coefficients are eliminated, ps may not contain max_x anymore
            if (x < max_x)
                add_cell_lits(ps, x);
        ...
        */
        ...
        // wzh Equational Constraints
        void collect_vars(polynomial_ref_vector const & ps){
            m_vars.reset();
            ...
            for(unsigned i = 0; i < ps.size(); i++){
                ...
                for(var v: curr_vars){
                    if(!m_vars.contains(v)){
                        m_vars.push_back(v);
                        m_max_var = (m_max_var == UINT_MAX || v > m_max_var) ? v : m_max_var;
                    }
                }
            }
        ...
        void mk_var_index(){
            m_index.reset();
            m_index.resize(m_max_var + 1, UINT_MAX);
        ...
        /**
         * @brief Select all equational constraints from literals
         * 
         * @param res store in res
         */
        void select_all_eqs(unsigned num, literal const * ls, polynomial_ref_vector & res){
            res.reset();
            for(unsigned i = 0; i < num; i++){
                literal l = ls[i];
                if(l.sign()){
        ...
        */
        void main(unsigned num, literal const * ls) {
            ...
            var max_x = UINT_MAX;
            // wzh nlcac
            cinterval_vector cac_vec;
            if(m_nlcac){
                m_cm.collect_cac_polys(num, ls, m_ps, m_atoms);
                max_x = max_var(m_ps);
                cac_vec = m_cm.get_unsat_cintervals(num, ls, m_atoms, max_x);
                TRACE("wzh", tout << "[nlcac] show cintervals: \n";
                    m_cm.display(tout, cac_vec);
                    tout << std::endl;
                );
            }
            else{
            ...
            // wzh Equational Constraints
            if(m_project_eq){
                select_all_eqs(num, ls, m_eqs);
                TRACE("nlsat_explain", tout << "[equal project] Equational Constraints: \n"; display(tout, m_eqs); tout << "\n";);
            }
            // hzw Equational Constraints
            TRACE("nlsat_explain", tout << "polynomials in the conflict:\n"; display(tout, m_ps); tout << "\n";);
            TRACE("wzh", tout << "[nlcac] current assignment:\n";
                m_assignment.display(tout);
                tout << std::endl;
            );

            // wzh: in conflict with nlcac
            if(!m_nlcac){
                elim_vanishing(m_ps);
            }

            if(m_project_eq){
                elim_vanishing(m_eqs);
            }
            TRACE("nlsat_explain", tout << "elim vanishing\n"; display(tout, m_ps); tout << "\n";);
            TRACE("nlsat_explain", tout << "[equal project] elim vanishing for Equational Constraints: \n"; display(tout, m_eqs); tout << "\n";);

            TRACE("wzh", tout << "[nlcac] show result lemma in main: " << std::endl;
                if(m_result == nullptr){
                    tout << "empty scoped literal vector" << std::endl;
                }
                else{
            ...
            // wzh projection choices
            if(m_project_eq){
            // if(false){
                TRACE("wzh", tout << "[equal project] use EQ projection" << std::endl;);
                collect_vars(m_ps);
                TRACE("wzh", tout << "[equal project] Heuristic Equational Constraints" << std::endl;);
                m_chosen_eqs.resize(m_vars.size());
                m_equal.heuristic_equal(m_eqs, m_chosen_eqs, m_vars, m_index);
                TRACE("wzh", tout << "[equal project] enter project EQ\n";);
                project_EQ(m_ps, m_chosen_eqs, max_x);
            }
            else if(m_nlcac){
                TRACE("wzh", tout << "[nlcac] use nlcac projection" << std::endl;);
                collect_vars(m_ps);
                TRACE("wzh", tout << "[nlcac] enter nlcac projection" << std::endl;);
                TRACE("wzh", tout << "[nlcac] enter nlcac projection" << std::endl;);
                project_nlcac(cac_vec, max_x);
                TRACE("wzh", tout << "[nlcac] exit nlcac projection" << std::endl;);
                TRACE("wzh", tout << "[nlcac] exit nlcac projection" << std::endl;);
            }
            else if(m_sample){
                TRACE("wzh", tout << "[sample] use sample projection" << std::endl;);
                collect_vars(m_ps);
                TRACE("wzh", tout << "[sample] enter sample projection" << std::endl;);
                project_sample(m_ps, max_x);
                TRACE("wzh", tout << "[sample] exit sample projection" << std::endl;);
            }
            else if(m_partial_project){
                TRACE("wzh", tout << "[partial] use partial projection\n";);
                collect_vars(m_ps);
                ...
                // elim_vanishing(m_ps);
                TRACE("wzh",
                    for(unsigned vi = 0; vi < m_vars.size(); vi++){
                        tout << "(" << vi << ", " << m_vars[vi] << ")" << std::endl;
                    }
                );
                TRACE("wzh", tout << "[project] use Traditional projection" << std::endl;);
                project(m_ps, max_x);
            }
            // hzw projection choices
        ...
        void operator()(unsigned num, literal const * ls, scoped_literal_vector & result) {
            ...
            TRACE("wzh", tout << "[nlcac] show result lemma finally: " << std::endl;
                if(m_result == nullptr){
                    tout << "empty scoped literal vector" << std::endl;
                }
                else{
