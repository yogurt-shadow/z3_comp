...
namespace nlsat {
    ...
    struct solver::imp {
        ...
        typedef std::pair<unsigned, interval_set *> undef_literal;
        typedef vector<undef_literal> undef_literal_vector;

        vector<undef_literal_vector> decision_index;
        // hzw
        ...
        imp(solver& s, ctx& c):
            ...
            // wzh
            m_cm(s, m_am, m_pm, m_evaluator, m_ism, m_cache, LAZARD)
            // hzw
            {
        ...
        */
        ...
        // wzh Equational Constraints
        void add_equational_clause(unsigned num_lits, literal const * lits, bool learned, assumption a){
            _assumption_set as = nullptr;
            if (a != nullptr)
                as = m_asm.mk_leaf(a);
            clause * cls = mk_clause_core(num_lits, lits, learned, as);
            m_lemma_count++;
            TRACE("wzh", tout << "[equal project] Equational lemma learnt" << std::endl;
                display(tout, *cls);
                tout << std::endl;
            );
        ...
        */
        ...
        // wzh decide interval
        // TODO: decide literal
        // output: return the decision index
        vector<unsigned> preprocess_clauses(clause_vector const & cs){
            // we first collect undef literals
            TRACE("wzh", tout << "[decide interval] enter collect undef literal\n";);
            collect_undef_literal(cs);
            TRACE("wzh", tout << "[decide interval] exit collect undef literal\n";);
            TRACE("wzh", tout << "[decide interval] show choices:\n";
                for(auto ele: decision_index){
                    TRACE("wzh", std::cout << "[decide loop] new vector\n";);
                    for(auto ele2: ele){
                        TRACE("wzh", std::cout << "[decide interval] show element: " << ele2.first << std::endl;);
                        TRACE("wzh", std::cout << "[decide interval] show infeasible set: ";
                            m_ism.display(std::cout, ele2.second);
                            std::cout << std::endl;
                        );
                    }
                }
            ...
            vector<unsigned> res;
            interval_set_ref curr_set(m_ism);
            dfs(decision_index, res, 0, curr_set);
            if(res.empty()){
                TRACE("wzh", std::cout << "[decide interval] empty choices\n";);
            }
            else{
                TRACE("wzh", std::cout << "[decide interval] nonempty choices\n";);
            }
            return res;
        }

        bool dfs(vector<undef_literal_vector> const & choices, vector<unsigned> & res, unsigned index, interval_set * st){
            if(index == choices.size()){
                return true;
            }
            for(unsigned i = 0; i < choices[index].size(); i++){
                interval_set_ref curr(m_ism);
                curr = m_ism.mk_union(st, choices[index][i].second);
                if(m_ism.is_full(curr)){
                    continue;
                }
                res.push_back(choices[index][i].first);
                if(dfs(choices, res, index + 1, curr)){
                    return true;
                }
                res.pop_back();
            }
        ...
        void collect_undef_literal(clause_vector const & cs){
            decision_index.reset();
            for(clause * c: cs){
                undef_literal_vector curr = collect_undef_literal(*c);
                // we do not have choices here
                if(curr.empty()){
                    decision_index.reset();
                    return;
                }
                decision_index.push_back(curr);
            }
        }

        undef_literal_vector collect_undef_literal(clause const & cls){
            undef_literal_vector res;
            for(unsigned idx = 0; idx < cls.size(); idx++){
                literal l = cls[idx];
                if(value(l) == l_false){
                    continue;
                ...
                if(curr_set == nullptr){
                    TRACE("wzh", tout << "empty\n";);
                }
                if(m_ism.is_empty(curr_set)){
                    TRACE("wzh", tout << 123 << std::endl;);
                    TRACE("nlsat_inf_set", tout << "infeasible set is empty, found literal\n";);
                    res.reset();
                    res.push_back({idx, nullptr});
                    return res;
                ...
                res.push_back({idx, curr_set});
            }
            return res;
        }

        // clause * reprocess_clauses(clause_vector const & cs) {
        //     unsigned index = 0;
        //     for (clause* c : cs) {
        //         if (!reprocess_clause(*c, false, index++))
        //             return c;
        //     }
        //     return nullptr; // succeeded
        // }

        // we do not have a noncovering choice
        // currently select all first choices
        // bool reprocess_clause(clause const & cls, bool satisfy_learned, unsigned index){
        //     if (!satisfy_learned && m_lazy >= 2 && cls.is_learned()) {
        //         TRACE("nlsat", tout << "skip learned\n";);
        //         return true; // ignore lemmas in super lazy mode
        //     }
        ...
        void reprocess_clauses(clause_vector const & cs, vector<unsigned> const & decision){
            for(unsigned i = 0; i < cs.size(); i++){
                reprocess_clause(*cs[i], decision, i);
            }
        }

        bool find_infeasible_set(vector<undef_literal_vector> const & vec, unsigned index, unsigned idx, interval_set * res){
            if(index >= vec.size()){
                return false;
            }
            for(auto ele: vec[index]){
                if(ele.first == idx){
                    TRACE("wzh", tout << "[debug] loop find infeasible set\n";);
                    res = ele.second;
                    return true;
                }
            }
        ...
        void reprocess_clause(clause const & cls, vector<unsigned> const & decision, unsigned index){
            unsigned curr_index = decision[index];
            TRACE("wzh", tout << "[decide interval] decision index: " << curr_index << std::endl;);
            literal l = cls[curr_index];
            if(value(l) == l_true){
                return;
            }
            interval_set_ref curr_set(m_ism);
            // TODO: cache
            // if(!find_infeasible_set(decision_index, index, curr_index, curr_set)){
            //     UNREACHABLE();
            // }
            atom * a = m_atoms[l.var()];
            curr_set = m_evaluator.infeasible_intervals(a, l.sign(), &cls);
            ...
            // unit clause
            if(decision_index[index].size() == 1){
                assign(l, mk_clause_jst(&cls)); 
                updt_infeasible(decision_index[index][curr_index].second);
                return;
            }
            // decide
            decide(l);
        ...
        */
            ...
            while (true) {
                ...
                while (true) {
                    ...
                    if (m_xk == null_var)
                        ...
                        // wzh decide interval
                        // currently use dfs
                        TRACE("wzh", tout << "[decide interval] enter preprocess clauses\n";);
                        auto decision_choices = preprocess_clauses(m_watches[m_xk]);
                        TRACE("wzh", tout << "[decide interval] exit preprocess clauses\n";);
                        if(decision_choices.empty()){
                        // if(true){
                            TRACE("wzh", std::cout << "decision is empty\n";);
                            // conflict_clause = reprocess_clauses(m_watches[m_xk]);
                            conflict_clause = process_clauses(m_watches[m_xk]);
                        }
                        else {
                            TRACE("wzh", std::cout << "not empty\n";);
                            TRACE("wzh", std::cout << "[decide interval] choices: ";);
                            for(auto ele: decision_choices){
                                TRACE("wzh", std::cout << ele << " ";);
                            }
                            TRACE("wzh", std::cout << std::endl;);
                            TRACE("wzh", std::cout << "[decide interval] enter reprocess clauses\n";);
                            reprocess_clauses(m_watches[m_xk], decision_choices);
        ...
        // wzh cdcac
        bool is_conjunction(){
            // std::cout << "size of m_watches: " << m_watches.size() << std::endl;
            // std::cout << "size of m_bwatches: " << m_bwatches.size() << std::endl;
            // std::cout << "number of bool vars: " << m_num_bool_vars << std::endl;
            // std::cout << "number of atoms: " << m_atoms.size() << std::endl;
            // for(unsigned i = 0; i < m_bwatches.size(); i++){
            //     for(clause * cls: m_bwatches[i]){
            //         display(std::cout, *cls);
            //         std::cout << std::endl;
            //     }
            // }
            for(unsigned i = 0; i < num_vars(); i++){
                for(clause * cls: m_watches[i]){
                    display(std::cout, *cls);
                    std::cout << std::endl;
                    if(cls->size() != 1 || cls->assumptions() != nullptr){
                        return false;
                    }
                }
            }
        ...
        lbool cdcac_main(){
            // tout << "we use cdcac\n";
            return get_unsat_cover(0).empty() ? l_true : l_false;
        }

        cinterval_vector get_unsat_cover(var x){
            TRACE("wzh", tout << "[cdcac] entering get unsat cover for var: " << x << std::endl;);
            interval_set_ref curr_infeasible(m_ism);
            cinterval_vector vec = get_unsat_intervals(x, curr_infeasible);
            TRACE("wzh", tout << "[cdcac] show cinterval vectors:\n";
                m_cm.display(tout, vec);
                tout << std::endl;
            );

            // curr_infeasible = m_cm.mk_union(vec);

            TRACE("wzh", tout << "[cdcac] show union interval set:\n";
                m_ism.display(tout, curr_infeasible);
                tout << std::endl;
            );
            ...
            while(!m_ism.is_full(curr_infeasible)){
                anum w;
                m_ism.peek_in_complement(curr_infeasible, m_is_int[x], w, m_randomize);
                if (!m_am.is_rational(w)){
                    m_irrational_assignments++;
                }
                m_assignment.set_core(x, w);
                TRACE("wzh", tout << "[cdcac] show assignment\n";
                    display_assignment(tout);
                );

                TRACE("wzh", tout << "[debug] show vec\n";
                    m_cm.display(tout, vec);
                    tout << std::endl;
                );
                ...
                cinterval_vector next = get_unsat_cover(x + 1);
                
                TRACE("wzh", tout << "[cdcac] show cinterval vector from next level with var: " << x + 1 << std::endl;
                    m_cm.display(tout, next);
                    tout << std::endl;
                );
                
                if(next.empty()){
                    TRACE("wzh", tout << "[cdcac] cinterval vector is empty, return sat\n";);
                    return next;
                }
                TRACE("wzh", tout << "[cdcac] enter construct characterization\n";
                    display_assignment(tout);
                    tout << std::endl;
                );
                ...
                TRACE("wzh", tout << "[cdcac] show construct cintervals\n";
                    m_cm.display(tout, next);
                    tout << std::endl;
                );
                m_cm.construct_characterization(next, ps, x + 1, m_assignment);
                
                TRACE("wzh", tout << "[cdcac] show construct characterization: \n";
                    m_cm.display(tout, ps);
                    tout << std::endl;
                    tout << "x is: " << x << std::endl;
                );
                ...
                m_am.set(w, m_assignment.value(x));
                m_assignment.reset(x);
                TRACE("wzh", tout << "[cdcac] after unset assignment\n";
                    display_assignment(tout);
                    tout << std::endl;
                );
                cinterval new_interval = m_cm.interval_from_characterization(ps, x, w, m_assignment);

                interval_set_ref new_set(m_ism);
                new_set = m_ism.mk(new_interval.m_lower_open, new_interval.m_lower_inf, new_interval.m_lower, new_interval.m_upper_open, new_interval.m_upper_inf, new_interval.m_upper, null_literal, nullptr);
                
                TRACE("wzh", tout << "[cdcac] show new interval:\n";
                    m_cm.display(tout, new_interval);
                    tout << std::endl;
                );
                TRACE("wzh", tout << "[cdcac] before push back\n";
                    m_cm.display(tout, vec);
                    tout << std::endl;
                );
                vec.push_back(new_interval);
                TRACE("wzh", tout << "[cdcac] after push back\n";
                    m_cm.display(tout, vec);
                    tout << std::endl;
                );
                curr_infeasible = m_ism.mk_union(curr_infeasible, new_set);
            }
            m_cm.prune_redundant_cintervals(vec);
            return vec;
        }

        cinterval_vector get_unsat_intervals(var x, interval_set_ref & infeasible_set){
            cinterval_vector res;
            infeasible_set.reset();
            interval_set_ref curr(m_ism);
            for(clause * cls: m_watches[x]){
                TRACE("wzh", tout << "[cdcac] consider clause: \n";
                    display(tout, *cls);
                );
                const clause & cur_cls = *cls;
                literal l = cur_cls[0];
                atom * a = m_atoms[l.var()];
                TRACE("wzh", tout << "[cdcac] enter get unsat cintervals\n";);
                cinterval_vector vec = m_cm.get_unsat_cintervals(l, m_atoms, x);
                TRACE("wzh", tout << "[cdcac] exit get unsat cintervals\n";);
                TRACE("wzh", tout << "[cdcac] show this unsat interval:\n";
                    m_cm.display(tout, vec);
                );
                for(unsigned i = 0; i < vec.size(); i++){
                    cinterval ele = vec.get(i);
                    res.push_back(ele);
                }
                TRACE("wzh", tout << "[cdcac] show cinterval vectors during loop\n";
                    m_cm.display(tout, res);
                );
                curr = m_evaluator.infeasible_intervals(a, l.sign(), cls);
                infeasible_set = m_ism.mk_union(infeasible_set, curr);
            }
            TRACE("wzh", tout << "[cdcac] show cinterval vectors before prune\n";
                    m_cm.display(tout, res);
            );
            m_cm.prune_redundant_cintervals(res);
            TRACE("wzh", tout << "[cdcac] show cinterval vectors after prune\n";
                    m_cm.display(tout, res);
            );
            return res;
        }
        ...
        lbool check() {
            ...
            // wzh cdcac entry (just for conjunction)
            if(m_cdcac){
        ...
        void resolve_lazy_justification(bool_var b, lazy_justification const & jst) {
            ...
            m_explain(jst.num_lits(), jst.lits(), m_lazy_clause, m_infeasible);
            // tout << "exit explain\n";
            for (unsigned i = 0; i < sz; i++)
        ...
        */
        ...
        start:
            ...
            TRACE("nlsat_resolve_done", tout << "[partial] resolve done, show assignment:\n";
                display_assignment(tout);
                tout << std::endl;
                tout << "[partial] current max var: " << max_var(m_assignment);
            );
            return true;
        }

        // wzh partial
        var max_var(assignment const & ass){
            for(int i = num_vars(); i >= 0; i--){
                if(ass.is_assigned(i)){
                    return i;
                }
            }
            return UINT_MAX;
    ...
    // wzh Equational Constraints
    void solver::add_equational_clause(unsigned num_lits, literal const * lits, bool learned, assumption a){
        m_imp->add_equational_clause(num_lits, lits, learned, a);   
    }
    // hzw Equational Constraints
